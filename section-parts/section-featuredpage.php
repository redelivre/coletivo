...
<?php $thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post_id), 'full' );?>
        <section style="background:url(<?php echo $thumb[0]; ?>) center no-repeat;     background-size: 100%;" id="<?php if ($onepress_featuredpage_id != '') {
    ...
    <div class="content"> 
    <?php else :
    echo '<div class="section-title-area"><br/><br/><h5 class="section-subtitle">You need to select a Featured Image for a background in full size</h5></div>';
    endif;?>
    <?php } ?>
