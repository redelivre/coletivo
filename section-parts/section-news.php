...
?>
<?php if ( ! $coletivo_news_disable  ) :

$desc = get_theme_mod( 'coletivo_news_desc' );
?>
<?php if ( ! coletivo_is_selective_refresh() ){ ?>
<section id="<?php if ( $coletivo_news_id != '' ) echo $coletivo_news_id; ?>" <?php do_action( 'coletivo_section_atts', 'news' ); ?> class="<?php echo esc_attr( apply_filters( 'coletivo_section_class', 'section-news section-padding onepage-section', 'news' ) ); ?>">
<?php } ?>
    <?php do_action( 'coletivo_section_before_inner', 'news' ); ?>
	<div class="container">
		<?php if ( $coletivo_news_title ||  $coletivo_news_subtitle ||  $desc ) { ?>
		<div class="section-title-area">
			<?php if ( $coletivo_news_subtitle != '' ) echo '<h5 class="section-subtitle">' . esc_html( $coletivo_news_subtitle ) . '</h5>'; ?>
			<?php if ( $coletivo_news_title != '' ) echo '<h2 class="section-title">' . esc_html( $coletivo_news_title ) . '</h2>'; ?>
            <?php if ( $desc ) {
			...
			<div class="row">
				<div class="col-sm-12">
					<div class="blog-entry wow slideInUp">
						...
						<?php if ( $coletivo_news_more_link != '' ) { ?>
						<div class="all-news">
							<a class="btn btn-theme-primary btn-lg" href="<?php echo esc_url($coletivo_news_more_link) ?>"><?php if ( $coletivo_news_more_text != '' ) echo esc_html( $coletivo_news_more_text ); ?></a>
						</div>
						<?php } ?>
	...
	<?php do_action( 'coletivo_section_after_inner', 'news' ); ?>
<?php if ( ! coletivo_is_selective_refresh() ){ ?>
</section>
<?php } ?>
<?php endif;
