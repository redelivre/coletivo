...
foreach ( $_images as $m ) {  // phpcs:ignore
	$m  = wp_parse_args( $m, array( 'image' => '' ) ); // phpcs:ignore
...
<?php if ( ! $coletivo_hero_disable && ! empty( $images ) ) { ?>
	...
	<?php
	if ( '' !== $coletivo_hero_id ) {
		echo esc_attr( $coletivo_hero_id );
	}
	?>
				...
				<div class="container"<?php echo esc_attr( $hero_content_style ); ?>>
					<div class="hero__content hero-content-style<?php echo esc_attr( $layout ); ?>">
						<div class="col-md-12 col-lg-6">
							<?php
							if ( $hcl2_content ) {
								echo '<div class="hcl2-content">' .apply_filters( 'the_content', wp_kses_post( $hcl2_content ) ). '</div>'; // phpcs:ignore
							}
							?>
						</div>
						<div class="col-md-12 col-lg-6">
			...
			default:
				...
				<div class="container"<?php echo esc_attr( $hero_content_style ); ?>>
					<div class="hero__content hero-content-style<?php echo esc_attr( $layout ); ?>">
						...
						if ( '' !== $hcl1_smalltext ) {
							echo '<p class="hero-small-text"> ' . wp_kses_post( $hcl1_smalltext ) . '</p>';
						}

						if ( '' !== $hcl1_btn1_text && '' !== $hcl1_btn1_link ) {
							echo '<a href="' . esc_url( $hcl1_btn1_link ) . '" class="btn ' . esc_attr( $btn_1_style ) . ' btn-lg">' . wp_kses_post( $hcl1_btn1_text ) . '</a>';
						}
		...
		if ( ! empty( $images ) && ! $is_parallax ) {
			...
			<script>
				jQuery(document).ready(function () {
					jQuery( ' .hero-slideshow-wrapper' ).backstretch(<?php echo wp_json_encode( $images ); ?>, {
						fade: 750,
						duration: 5000
					});
				});
